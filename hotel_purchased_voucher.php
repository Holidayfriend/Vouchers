...
$page_text = getTranslation('all_purchased_voucher', $lang);
...
$sql = "SELECT a.*,b.title FROM `tbl_users_vouchers` as a INNER JOIN tbl_voucher as b On a.`voucher_id` = b.id Where a.`user_id` = $my_user_id_is AND a.`is_delete` = 0
...
$countSql = "SELECT COUNT(*) as  total FROM `tbl_users_vouchers` as a INNER JOIN tbl_voucher as b On a.`voucher_id` = b.id Where a.`user_id` = $my_user_id_is 
AND a.`is_delete` = 0
...
<head>
    ...
    <title><?= getTranslation('all_purchased_voucher', $lang) ?></title>
    <title></title>
    <!-- This page CSS -->
    <!-- chartist CSS -->
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $("#selectAll").change(function () {
                ...
                if (isChecked) {
                    $(".single_div_io").each(function () {
                        var id = $(this).attr("id");
                        $(this).addClass("selected");
                        selectedIds.push(id);
                    });
                } else {
                    $(".single_div_io").removeClass("selected");
                }

...
$page_text = getTranslation('vouchers', $lang);
...
if (!empty($search)) {
    $searchCondition = "AND (`title` = '$search' OR `description` LIKE '%$search%')";
} else {
    $searchCondition = '';
}
...
if ($cityFilter !== 'all') {
    $filterCondition .= " AND 	`cat_id` = '$cityFilter'  ";
}
...
$sql = "SELECT * FROM `tbl_voucher` WHERE `user_id` =  $my_user_id_is AND is_delete = 0 $searchCondition $filterCondition ";
$result = $conn->query($sql);
$sql_mobile = $sql;

// Query to count total records with search
$countSql = "SELECT COUNT(*) as total FROM `tbl_voucher`  WHERE `user_id` =  $my_user_id_is AND is_delete = 0 $searchCondition $filterCondition";
...
<head>
    ...
    <title><?= getTranslation('vouchers', $lang) ?></title>
    <!-- This page CSS -->
    <!-- chartist CSS -->
...
<body class="skin-default-dark fixed-layout mini-sidebar lock-nav">
    ...
    <div class="preloader">
        <div class="loader">
            ...
            <p class="loader__label"><?= getTranslation('vouchers', $lang) ?></p>
        </div>
    </div>
    <!-- ============================================================== -->
                                    ...
                                    <form id="filterForm">
                                        ...
                                        <div class="form-group mt-3">
                                            ...
                                            <select class="form-control" id="cityFilter" name="cityFilter">
                                                <option value="all" <?php echo ($cityFilter === 'all') ? 'selected' : ''; ?>><?= getTranslation('category', $lang) ?></option>
                                                <?php
                                                ...
                                                if ($result && $result->num_rows > 0) {
                                                    while ($row = mysqli_fetch_array($result)) {
                                                        $selected = ($row[0] == $cityFilter) ? "selected" : "";
                                                        ?>
                                                        <option value="<?php echo $row[0]; ?>" <?php echo $selected; ?>>
                                                            <?php $langKey = $lang == 'en' ? 'name' : 'name_' . $lang;
                                                            echo isset($row[$langKey]) ? $row[$langKey] : $row['name'];
                                                            ?>
                                                        </option>
                                                        <?php
                                                    }
                    ...
                    <div class="row page-titles mb-3  ">
                        <div class="col-md-12">
                            <div class="row mt-3">
                                <div class="col-md-6">
                                    <span id="rowCount"><b
                                            class="text_black_color"><?= getTranslation('vouchers_list', $lang) ?>(</b><b
                                ...
                                <div class="col-md-6 text-right">
                                    ...
                                    <span id="deleteButton" class="pl-2 pr-2 text_black_color pointer"><i
                                            class="f_hover fas fa-trash-alt"></i>&nbsp;<?= getTranslation('delete', $lang) ?></span>
                                    <span data-toggle="modal" data-target="#filterModal" id=""
                                        class="f_hover pl-2 pr-2 text_black_color pointer"> <i
                                            class="fas fa-filter"></i>&nbsp;<?= getTranslation('filter', $lang) ?></span>
                                    <button onclick="url_redirect('hotel_add_vochers.php')" class="btn btn-add"
                                        id="addButton"> <i
                                            class="mdi mdi-plus"></i><?= getTranslation('add_vouchers', $lang) ?></button>
                                </div>
                            </div>
                            <div class="row mt-3">
                                ...
                                if (mysqli_num_rows($result_mobile) > 0) {
                                    while ($row = mysqli_fetch_array($result_mobile)) {
                                        ...
                                        $campany_status = $row['status'];
                                        if ($campany_status == 'ACTIVE') {
                                            $status_class = 'span_status_green';
                                            $status_text = getTranslation('activated', $lang);
                                            $action_class = 'btn_deactive';
                                            $action_text = getTranslation('deactivated', $lang);
                                            $pass = getTranslation('deactive', $lang);

                                        } else {
                                            $status_class = 'span_status_red';
                                            $status_text = getTranslation('deactivated', $lang);
                                            $action_class = 'btn_active';
                                            $action_text = getTranslation('activated', $lang);
                                            $pass = getTranslation('active', $lang);
                                        }
                                        $id = $row['id'];
                                        ...
                                        $cat_name = '';
                                        if ($cat_id > 0) {
                                            $sql_c = "SELECT `name`, `name_it`, `name_de`, `is_fixed` FROM `tbl_category` WHERE `id` = $cat_id";
                                            ...
                                            if ($row_c = mysqli_fetch_assoc($result_c)) {
                                                ...
                                                if ($lang == 'en') {
                                                    ...
                                                    if (!empty($row_c['name'])) {
                                                        $cat_name = $row_c['name'];
                                                    } elseif (!empty($row_c['name_it'])) {
                                                        $cat_name = $row_c['name_it'];
                                                    } elseif (!empty($row_c['name_de'])) {
                                                        $cat_name = $row_c['name_de'];
                                                    }
                                                } else if ($lang == 'it') {
                                                    if (!empty($row_c['name_it'])) {
                                                        $cat_name = $row_c['name_it'];
                                                    } elseif (!empty($row_c['name'])) {
                                                        $cat_name = $row_c['name'];
                                                    } elseif (!empty($row_c['name_de'])) {
                                                        $cat_name = $row_c['name_de'];
                                                    }
                                        ...
                                        <div class="col-lg-4 ">
                                            ...
                                            <div onclick="toggleSelect(<?php echo $id; ?>)" id="<?php echo $id; ?>"
                                                class="row single_div_io pointer">
                                                <div class="col-lg-12 ">
                                                    <div class="row">

                                                        <div class="col-12 ">
                                                            <img src="<?php echo $image; ?>" alt="Image" class="w-100">
                                                        </div>



                                                        <div class="col-12 mt-2">
                                                            <h5 class="dark_gray_color"><b><?php echo $title; ?></b> </h5>
                                                        </div>

                                                        <div class="col-12 text-right ">

                                                            <a href="hotel_edit_vochers.php?id=<?php echo $id; ?>">
                                                                <span class="edit_hover single_div__dark_text"> <i
                                                        ...
                                                        <div class="col-12 mt-2">
                                                            <h6 class="dark_gray_color">
                                                                <b> <?php
                                                                echo getTranslation('categoey', $lang);
                                                                echo ':' . $cat_name; ?></b>
                                                            </h6>
                                                        </div>
                                                    ...
                                                    <div class="row mt-3">
                                                        <div class="col-6 ">
                                                            <?php if ($active_from != '' && $active_from != '') { ?>
                                                                <span
                                                                    class="expiry_text"><?= getTranslation('activate_period', $lang) ?></span>
                                                                <br>

                                                                <span
                                                                    class="expiry_date_text"><?php echo $active_from . ' to ' . $active_to; ?></span>
                                                            <?php } else {
                                                            } ?>
                                                        </div>
                                                        <div class="col-6 text-right mt-3">
                                                            <span
                                                                onclick="action(event,<?php echo $id ?>,'<?php echo $pass ?>','<?php echo $title ?>')"
                                                                class="pointer <?php echo $action_class ?> "><?php echo $action_text ?></span>
                                                        </div>
                                                    </div>
                                    ...
                                    <div style="text-align: center;">
                                        <h3 style="margin: 0; padding: 20px; color: #333;"><?= getTranslation('not_found', $lang) ?></h3>
                                    </div>
        ...
        $(document).ready(function () {
            ...
            $("#deleteButton").click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "utill/del.php",
                    data: { selectedRows: selectedIds, base: 'voucher', base_name: 'id' },
                    success: function (response) {
